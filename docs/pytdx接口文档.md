...
## 极限测试结果（2026-04-11 实测）

> 测试时间：2026-04-11，测试股票：平安银行(000001)、浦发银行(600000)，服务器：180.153.18.170:7709
...
### 2. 股票列表分页 (get_security_list)

| 市场 | 每页条数 | 最大有效偏移 | 备注 |
|------|---------|-------------|------|
| 深圳 | 1000 | ~6000 | offset=7000起返回空 |
| 上海 | 1000 | 特殊 | offset=0直接返回空，需从~800开始 |

### 3. K线 (get_security_bars) —— 最大条数

| category | 周期 | 最大返回条数 | 备注 |
|----------|------|-------------|------|
| 0/1/2/3 | 5/15/30分钟、1小时 | 未测试 | |
| 4 | 日线 | 未测试 | |
| 5 | 周线 | 未测试 | |
| 6 | 月线 | 未测试 | |
| 7/8 | 1分钟 | 未测试 | |
| **9** | **日线(通达信)** | **800条** | **超过800返回0条** |
| 10/11 | 季/年 | 未测试 | |

**结论**：A 股日线单次请求最多返回约 **800 条**。

### 4. K线 (get_security_bars) —— 时间跨度

以 category=9（日线）测试：

| 请求条数 | 实际返回 | 时间跨度 |
|---------|---------|---------|
| 800条 | 800条 | 约3年（2017-2020起视股票而异） |
| 1600条 | 0条 | 无法一次获取 |
| 2400条 | 0条 | 无法一次获取 |

**结论**：单次请求最多获取约 **800 条日线**，即约 **3年数据**。如需更长时间跨度，需分多次请求并使用 `start` 参数翻页。

### 5. K线成交量单位

| category | vol单位 | amount单位 | 判断依据 |
|----------|--------|-----------|---------|
| 4 (日线) | 股 | 元 | amount/vol ≈ 股价 |
| 9 (日线) | 手 | 元 | amount/vol ≈ 股价×100 |

**结论**：
- `category=4` 的 `vol` 单位为**股**
- `category=9` 的 `vol` 单位为**手**（1手=100股）
- 两种日线的 `amount` 单位均为**元**

### 6. K线除权问题

| category | 数据类型 | 是否除权 | 说明 |
|----------|---------|---------|------|
| 4 | 日线 | **前复权** | 数据显示的是前复权价格 |
| 9 | 日线(通达信) | **前复权** | 同上 |
| 5 | 周线 | **前复权** | |
| 6 | 月线 | **前复权** | |

**结论**：所有 K 线数据均为**前复权**数据。如需获取真实（未复权）价格，需通过 `get_xdxr_info` 获取除权除息记录自行计算。

### 7. 当日分时 (get_minute_time_data)

| 项目 | 结果 |
|------|------|
| 单次返回条数 | 全天约240-480条（每分钟1条） |
| vol单位 | **手** |
| 与快照vol对比 | 分时vol合计 ≈ 快照vol（误差<5%） |

### 8. 历史分时 (get_history_minute_time_data)

| date | 结果 | 备注 |
|------|------|------|
| 20260301 | **无数据** | 可能需要更近的日期 |
| 20260101 | 无数据 | |
| 20250101 | 无数据 | |
| 更早 | 无数据 | |

**结论**：历史分时数据**仅支持近几日**，无法获取长期历史分时。

### 9. 当日逐笔 (get_transaction_data)
...
**结论**：单次请求最多约 **500 条**逐笔数据。

### 10. 历史逐笔 (get_history_transaction_data)

| date | 结果 | 备注 |
|------|------|------|
| 20260410 | **有数据** | 10条/次，需分页获取全量 |
| 20250410 | **有数据** | 一年前的数据仍可获取 |
| 20240410 | **有数据** | 两年前的数据仍可获取 |
| 20230410 | **有数据** | 三年前的数据仍可获取 |
| 20220801 | **有数据** | 约4年前的数据仍可获取 |
| 非交易日 | 无数据 | 周末/节假日返回空 |

**结论**：历史逐笔数据可追溯至约 **4 年前**（2022 年），远非"仅近几日"。之前测试中"无数据"的日期实际为非交易日。非交易日（周末、节假日）返回空数据属于正常行为。

### 11. 财务信息 (get_finance_info)

| 项目 | 结果 |
|------|------|
| 返回数据 | 有数据（37字段） |
| updated_date | 最近更新日期（如20260321） |
| liutongguben单位 | **股** |
| zongguben单位 | **股** |

### 12. 除权除息 (get_xdxr_info)

| 市场 | 记录数 | 最早年份 | 除权类型 |
|------|-------|---------|---------|
| 深圳 | 78条 | 1990年 | 除权除息、股本变化、送配股上市等 |
| 上海 | 87条 | 1999年 | 同上 |

### 13. 指数K线 (get_index_bars)

| category | 是否支持 | 额外字段 |
|----------|---------|---------|
| 8 (1分钟) | 支持 | 包含up_count/down_count |
| 9 (日线) | 支持 | 包含up_count/down_count |
| 其他 | 未测试 | |

### 14. 板块信息 (get_block_info_meta)

| blockfile | size | 备注 |
|-----------|------|------|
| block.dat | 281,686 | |
| block_gn.dat | 757,083 | 概念板块 |
| block_zs.dat | 326,694 | 指数板块 |
| block_fg.dat | 453,279 | 风格板块 |

---

### 极限测试总结

| 接口 | 最大条数 | 时间跨度 | 除权 | vol单位 |
|------|---------|---------|------|--------|
| get_security_bars(日线) | 800条/次 | ~3年 | 前复权 | category=4:股, category=9:手 |
| get_minute_time_data | 全天240-480条 | 当日 | N/A | 手 |
| get_history_minute_time_data | - | **仅近几日** | N/A | 手 |
| get_transaction_data | ~500条/次 | 当日 | N/A | 手 |
| get_history_transaction_data | ~500条/次 | **约4年** | N/A | 手 |
| get_security_list | 1000条/页 | - | N/A | N/A |
| get_finance_info | 1条 | 最新 | N/A | 股 |
| get_xdxr_info | 78-87条 | 1990/1999起 | N/A | N/A |
| get_index_bars | 800条/次 | ~3年 | 前复权 | 手 |
...
- 返回：`list[OrderedDict]`，字段意义：
  ...
  - `num`：撮合笔数（该条记录由多少笔相同价格的同时成交撮合而成，`vol` 为撮合后的总量，`vol/num` 不固定）
  - `buyorsell`：买卖方向标识（协议原始字段，pytdx 不做枚举映射）
    - A 股实测：取值为 `0/1/2/8`
    - **自验证结论（2026-04-12 实测，验证日期 20260410/20250410，验证股票：平安银行/贵州茅台/网宿科技/分众传媒）**：
      - `0`：**主动买入** — 当 `buyorsell=0` 时，对应逐笔的价格上涨概率显著高于其他值（平安银行 48.8% vs 0.1%，茅台 73.2% vs 12.8%）；涨停日 bs=0 金额远大于 bs=1（分众传媒涨停日 bs0-bs1=+1.49亿）
      - `1`：**主动卖出** — 当 `buyorsell=1` 时，对应逐笔的价格下跌概率显著高于其他值；大跌日 bs=1 金额远大于 bs=0（网宿科技 -9.77%日 bs0-bs1=-12.90亿）
      - `2`：**中性/撮合成交** — 笔数极少（全天约 10 笔），但单笔极大（平均约 1057 手 vs 正常的 112 手），集中在集合竞价时段和尾盘大额撮合
      - `8`：**集合竞价标记** — 仅出现在 `vol=0` 的记录中，集中在 09:15-09:25 和 14:57-14:59 时段，属于集合竞价期间的挂单/撤单/撮合记录（非实际成交），`buyorsell=8` 的记录占全天逐笔约 1.7%~3.0%，建议在资金流分析中过滤（`vol=0` 过滤即可覆盖）
    - 验证方法：三重自验证（逐笔价格变动方向统计、涨停/跌停极端场景验证、bs=2/8 行为特征分析），无需外部数据源，纯 pytdx 数据即可完成
...
- 返回：与 `get_transaction_data` 类似，但不含 `num` 字段：
  ...
  - A 股实测：`buyorsell` 取值及含义与 `get_transaction_data` 完全一致（见上方自验证结论）
