...
def run_screener_task(screener_id: int):
    ...
    try:
        ...
        if success and data:
            # Save result
            # Limit result size to avoid DB bloat? For now, allow it.
            ...
            subq = db.query(ScreenerResult.id).filter(ScreenerResult.screener_id == screener_id).order_by(ScreenerResult.run_at.desc()).offset(10).all()
            if subq:
