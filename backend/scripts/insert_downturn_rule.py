...
def insert_downturn_rule(script_path: str, force_update: bool):
    """将掉头向下监控脚本插入数据库"""
    ...
    try:
        ...
        if existing:
            ...
            if do_update:
                ...
                existing.description = "区分技术性调整与趋势反转：监测跌破MA20、放量杀跌等危险信号。"
                db.commit()
        ...
        rule = RuleScript(
            name=rule_name,
            description="区分技术性调整与趋势反转：监测跌破MA20、放量杀跌等危险信号。",
            code=script_content
        )
...
if __name__ == "__main__":
    ...
    default_path = os.path.join(os.path.dirname(__file__), "跟踪策略", "掉头向下监控.py")
    ...
    insert_downturn_rule(script_path=script_path, force_update=bool(args.force))
