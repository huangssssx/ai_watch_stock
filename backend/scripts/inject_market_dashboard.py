...
def inject_dashboard():
    print("=== Injecting Market Dashboard Script ===")
...
'''
    ...
    try:
        # Check if exists
        title = "Data Lab - Market Sentiment Dashboard"
        existing = db.query(ResearchScript).filter(ResearchScript.title == title).first()
        
        if existing:
            print(f"Updating existing script: {title}")
            existing.script_content = streamlit_code
            existing.updated_at = datetime.datetime.now()
        else:
            print(f"Creating new script: {title}")
            new_script = ResearchScript(
                title=title,
                description="A-Share Market Sentiment Analysis (Streamlit)",
                script_content=streamlit_code
            )
            db.add(new_script)
