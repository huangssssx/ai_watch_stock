...
# 目标：修正成交量单位(手/股)导致的 VWAP 虚高 100 倍问题，恢复策略机能
...
# --- 主程序 ---
print("🎯 【山谷狙击选股策略 v8.6 (Bug修复版)】启动")
print("🛠️ 修复内容：VWAP 单位数量级自动修正")
print("📡 正在获取市场数据...")
...
if not df_market.empty:
    ...
    if "涨跌幅" in df_scan.columns:
        for idx, row in df_scan.iterrows():
            symbol = str(row["代码"])[2:] if len(str(row["代码"]))>=8 else str(row["代码"])
